...
\term{残差网络}{Residual Networks}（或ResNets）由He等人\cite{he2015}在2015年提出。在本章中，我们将讨论这些网络是什么，为什么引入它们以及它们与常微分方程的关系。
...
\node at (3,2) {无残差连接的MLP性能};
\node at (3,1) {(训练集)};

\draw (7,0) rectangle (13,4);
\node at (10,2) {有残差连接的MLP性能};
\node at (10,1) {(训练集)};
\end{tikzpicture}
...
% 跳跃连接
\path [skip] (x0) to[bend above] (x2);
\path [skip] (x1) to[bend above] (x3);
\path [skip] (x2) to[bend above] (x4);
\path [skip] (x3) to[bend above] (x5);
\path [skip] (x4) to[bend above] (x6);
...
% 定义节点样式
\tikzstyle{block} = [rectangle, draw, fill=blue!20, text width=1.5cm, text centered, minimum height=1cm]
...
\node [block, above of=sum1, node distance=1cm] (identity) {恒等映射};

% 连接
\path [line] (f1) -- (sum1);
\path [skip] (identity) -- (sum1);

% 输入输出
\node [left of=f1, node distance=2cm] (input) {$x^{(l-1)}$};
\node [right of=sum1, node distance=2cm] (output) {$x^{(l)}$};

\path [line] (input) -- (f1);
\path [line] (sum1) -- (output);
\path [skip] (input) |- (identity);

% 标签
\node [below of=f1, node distance=2cm] {残差网络的前向和反向传播计算图};
\end{tikzpicture}
...
受残差网络和常微分方程之间联系的启发，\term{神经常微分方程}{Neural ODEs}在\cite{chen2018}中被提出，该论文获得了NeurIPS 2018最佳论文奖。考虑由以下给出的常微分方程组
\begin{equation}
\frac{dx}{dt} = V(x, t)
...
% 定义节点样式
\tikzstyle{input} = [circle, draw, fill=green!20, minimum size=0.8cm]
\tikzstyle{hidden} = [circle, draw, fill=blue!20, minimum size=0.8cm]
\tikzstyle{output} = [circle, draw, fill=red!20, minimum size=0.8cm]
\tikzstyle{line} = [draw, -latex']

% 输入层
\node [input] (x) {$x$};
\node [input, below of=x] (t) {$t$};

% 隐藏层
\node [hidden, right of=x, node distance=2cm] (h1) {};
\node [hidden, above of=h1, node distance=0.8cm] (h2) {};
\node [hidden, below of=h1, node distance=0.8cm] (h3) {};
\node [hidden, right of=h1, node distance=1.5cm] (h4) {};
\node [hidden, above of=h4, node distance=0.8cm] (h5) {};
\node [hidden, below of=h4, node distance=0.8cm] (h6) {};

% 输出层
\node [output, right of=h4, node distance=2cm] (v) {$V(x,t)$};
...
% 标签
\node [below of=h1, node distance=2.5cm] {用于建模神经ODE右手边的前馈神经网络};
\node [below of=h1, node distance=3cm] {自变量的数量 = $d-1$};
\end{tikzpicture}
\caption{用于建模神经常微分方程右手边的前馈神经网络。自变量的数量=$d-1$}
...
\begin{lstlisting}[caption={神经ODE学习动力学系统的PyTorch实现}, label=code:neural_ode_example]
import torch
